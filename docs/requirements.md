...
|---|---|---|---|---|
| K01 | P0 | Boot UEFI ke init di QEMU; 100 cold boot berturut-turut tanpa panic | **verified** | `xtask soak --boots 100` → `docs/evidence/soak-summary.txt` (100/100, rata-rata 5,4 s per boot di TCG); skenario `acceptance` (marker `[init] Space OS init running`); diagnosis panic diuji oleh skenario `panic-diagnosis`, `kernel-fault-diagnosis`, `kernel-stack-overflow-diagnosis` |
