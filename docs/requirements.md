...
| U01 | P1 | Desktop/terminal/file manager/Stop tetap hidup saat worker inferensi crash | **experimental (ketahanan terbukti, UI belum)** | `bin/spaceshell` memiliki sesi konsol, daftar berkas (`SYS_FS_LIST`) dan masa hidup worker; loopnya tidak pernah memblokir pada worker (`recv` non-blocking + `SYS_WAIT` bendera `NONBLOCK`). Uji menjalankan empat worker berturut-turut — crash (`PAGE_FAULT`), macet tanpa syscall, tidur, dan selesai normal — dan menuntut sesi tetap menjawab `STATUS` serta membuka daftar berkas setiap kali, `STOP` berhasil pada worker macet maupun yang tidur. Sesi hanya memegang `SPAWN|FS` hasil penyempitan handle root. **Catatan jujur:** "desktop" masih konsol teks framebuffer dan **belum ada masukan keyboard ke user space** — perintah datang lewat channel kontrol, bukan dari orang yang mengetik. ADR-0011 |
