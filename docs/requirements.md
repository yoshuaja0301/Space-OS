...
|---|---|---|---|---|
| K01 | P0 | Boot UEFI ke init di QEMU; 100 cold boot berturut-turut tanpa panic | **verified** | `xtask soak --boots 100` → `docs/evidence/soak-summary.txt`; skenario `acceptance` (marker `[init] Space OS init running`); diagnosis panic diuji oleh skenario `panic-diagnosis`, `kernel-fault-diagnosis`, `kernel-stack-overflow-diagnosis` |
| K02 | P0 | User-space, syscall, IPC, timer, isolasi; akses terlarang mematikan proses uji, bukan kernel | **verified** | `init` menjalankan: tulis ke memori kernel, baca NULL, eksekusi stack NX, `div`, `ud2`, `cli` → proses dibunuh dengan alasan yang benar, kernel lanjut; 30 uji negatif ABI (`bin/abi_negative`); IPC echo + transfer handle; `sleep(50 ms)`; proses runaway di-preempt dan di-kill |
| K03 | P0 | Kuota dan reclamation; siklus berulang tanpa kebocoran yang terus tumbuh | **verified** | `bin/quota`: `mem_map` ditolak `Quota` tepat pada batas dan dapat dipakai ulang; 50 siklus spawn/exit `bin/worker` (channel + 16 halaman + heap) dan 20 siklus kill-saat-blocking `bin/ipc_echo` → jumlah frame bebas dan heap kernel identik sebelum/sesudah (`[init] frames free before=… after=…`) |
