...
| File | Skenario | Hasil |
|---|---|---|
| `acceptance.log` | boot normal, `init` menjalankan 53 uji K01–K03, D01, C01, A01, U01, G01, L01–L03 dan P01 | `ALL TESTS PASSED (53/53, 0 skipped)`, exit QEMU 33; termasuk 128 token inferensi yang cocok dengan baseline, sesi yang bertahan melewati empat worker, agent yang tidak bisa keluar workspace, context bundle yang diverifikasi provenance-nya, dan paket yang ditolak dengan alasannya masing-masing. Ring masukan konsol juga diluapkan dengan sengaja di bawah sesi yang hidup: `console input: 8 byte(s) dropped` lalu `[shell] input was lost; the line was discarded` |
...
| `compat-no-disk.log` | mesin tanpa perangkat blok | `virtio-blk: no device present`, uji berbasis disk dilewati, `ALL TESTS PASSED (43/43, 10 skipped)` |
