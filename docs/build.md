...
| `cargo xtask test` | empat skenario boot + pemeriksaan log/exit code, log di `build/logs/` |
| `cargo xtask soak --boots 100` | 100 cold boot berturut-turut skenario acceptance (K01) |
| `cargo xtask clippy` / `fmt` / `fmt-check` / `ci` | lint dan format semua target |
...
EFI/SPACEOS/initrd.tar        bin/init, bin/hello, bin/fault, bin/abi_negative, bin/ipc_echo, bin/quota, bin/spin, bin/worker
