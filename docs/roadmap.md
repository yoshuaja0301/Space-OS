...
5. **Penyimpanan yang bisa ditulis** — **kernel selesai** (ADR-0015): FAT32 tulis, `SYS_FS_CREATE`/`SYS_FS_WRITE` di balik hak `FS_WRITE`, dan uji yang menuntut boot kedua menemukan apa yang ditulis boot pertama. Yang tersisa adalah memindahkan ketiga layanan ke sana: tambalan agent (G01), daftar revokasi (L02), dan store paket (P01) masih hidup di memori satu boot.
