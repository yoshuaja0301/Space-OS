...
5. **Penyimpanan yang bisa ditulis** — **kernel selesai** (ADR-0015): FAT32 tulis, `SYS_FS_CREATE`/`SYS_FS_WRITE` di balik hak `FS_WRITE`, dan uji yang menuntut boot kedua menemukan apa yang ditulis boot pertama. Store paket (P01) dan daftar revokasi (L02) sudah dipindahkan ke sana, masing-masing dibuktikan dengan proses kedua yang memuatnya dari disk. Yang tersisa: tambalan agent (G01) masih mendarat di overlay dalam memori.
